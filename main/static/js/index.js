    ...
    var store = Ext.create('Ext.data.TreeStore', {
        ...
        (function() {
                ...
                success: function(usernames){
                    ...
                    // msg.rate = 1;    
                    msg.pitch = 0.8;    
                    msg.text = usernames.responseText+' Successfully Logged in';    
                    msg.lang = 'en-US';     
                    speechSynthesis.speak(msg); 
                    usernames = usernames.responseText
                    mynorthpanel.html = "<div id='top'><div class='title'><h1>INTOUCH COMMUNICATIONS LTD</h1></div><div class='welcome'><p> Welcome, "+ usernames + " <a href='/logout'>Logout</a></p></div></div>"
                 }
            });
            })();
...
function userprofile() {
    ...
    Ext.Ajax.request({
        ...
        success: function(response){
            ...
            // title = result.response[0].fields.mobilephone
            profile_username = userprofilepanel.getComponent(0).getComponent(0).setValue(result[0].username)
            profile_firstname = userprofilepanel.getComponent(0).getComponent(1).setValue(result[0].first_name)
            profile_lastname = userprofilepanel.getComponent(0).getComponent(2).setValue(result[0].last_name)
            profile_email = userprofilepanel.getComponent(0).getComponent(3).setValue(result[0].email)
            profile_home_phone = userprofilepanel.getComponent(2).getComponent(0).setValue(result[0].homephone)
            profile_business_phone = userprofilepanel.getComponent(2).getComponent(1).setValue(result[0].businessphone)
            profile_mobile_phone = userprofilepanel.getComponent(2).getComponent(2).setValue(result[0].mobilephone)
...
function customers() {
    ...
    var customersgrid = Ext.create('Ext.grid.Panel', {
        ...
        columns: [
            ...
            {text: 'Mobile', dataIndex: 'telephone', flex: 20 / 100, align: 'right'},
        ...
        dockedItems: [{
            ...
            items: [{
                ...
                listeners: {
                    'keyup': function(f,e) {
                        customersgrid.store.getProxy().extraParams={pattern: f.getValue()};
                        customersgrid.store.loadPage(1);
                    }
                }
            }],
        },{
...
// func for customers related invoice items

   
        var customer_items = Ext.create('Ext.data.Store', {
            // storeId: 'customers_items_store',
            autoLoad: true,
            fields: [
                {name: 'id', mapping: 'fields.id'},
                {name: 'item_descri', mapping: 'fields.item_descri'},
                {name: 'quantity', type: 'int', mapping: 'fields.quantity'},
                {name: 'unit_price', mapping: 'fields.unitPrice'},
                {name: 'total_exclu_vat', mapping: 'fields.excluVat'},
                {name: 'vat_customer', mapping: 'fields.vatCustomer'},
                {name: 'total_customer_item', mapping: 'fields.totalCustomerItem'},
                {name: 'invo_connect_id', mapping: 'fields.invo_connect_id'}
            ],

            proxy: { 
                type: 'ajax', 
                url:'/customers/getcustomeritems',
                /*extraParams:{
                ...
                reader: { 
                    type: 'json',
                    root: 'response', // Must match the property in the JSON response in the back
...
function addCustomerItem(invoiceconnection){
    var addItemWin = Ext.create('Ext.window.Window', {
        ...
        items:[{
            ...
            buttons: [{
                ...
                formBind: true,
                handler: function() {
                    var details = addItemWin.getComponent(0)
                    var values = details.getValues()
                    console.log(values)
                    Ext.Ajax.request({
                            url: '/customers/addcustomeritem',
                            method: 'POST',
                            params: {
                                
                                description: values.additem_field,
                                quantity: values.item_quantity,
                                unit: values.item_unitprice,
                            ...
                            success: function() {
                                ...
                                customer_items.reload() 
                                
                            },
                            failure: function() {
                                Ext.Msg.alert('Failed', 'Failed to add an item')
                            }
                     });
                }
            },{
...
// Edit customer item
function editCustomerItem(itemPk, description, qty, unit, connection){
    var editItemWin = Ext.create('Ext.window.Window', {
        ...
        items:[{
            ...
            buttons: [{
                ...
                formBind: true,
                handler: function() {
                    var details = editItemWin.getComponent(0)
                    var values = details.getValues()
                    console.log(values)
                    Ext.Ajax.request({
                            url: '/customers/editcustomeritem',
                            method: 'POST',
                            params: {
                                item_id: itemPk,
                                description: values.editadditem_field,
                                quantity: values.edititem_quantity,
                                unit: values.edititem_unitprice,
                            ...
                            failure: function() {
                                Ext.Msg.alert('Failed', 'Failed to edit an item')
                            }
                     });
                }
            },{
...
function removeCustomerItem(itemdescri, itempk, connection){
    Ext.Msg.confirm('Delete ' + itemdescri, 'Are you sure you want to remove  ' + itemdescri + ' from your items', function(button) {

    if(button == 'yes') {
      Ext.Ajax.request({
        url: '/customers/removecustomeritem',
        method: 'POST',
        params: {
            customeritemkey: itempk,
            connection: connection,
        ...
        success: function() {
            ...
            customer_items.reload()
            Ext.Msg.alert('Success', 'Successfully removed Customer Item')
        },
        failure: function() {
            Ext.Msg.alert('Failed', 'Failed to remove Customer Item')
        }
...
var currencies = Ext.create('Ext.data.Store', {
    fields: ['name'],
    data : [
        {"name":"Rwandan Francs"},
        {"name":"US DOLLARS"},
...
function custInvoices() {
    ...
    /*customer_items = Ext.create('Ext.data.Store', {
        storeId: 'customers_items_store',
        autoLoad: true,
        fields: [
            {name: 'id', mapping: 'fields.id'},
...
*/
    /*var hot = customersinvoice_store.getComponent(2).getComponent(2).getComponent(3)
    console.log(hot)*/
    ...
    customersinvoice_store = Ext.create('Ext.data.Store', {
        ...
        fields: [
           ...
           {name: 'currency_inv', /*type: 'string',*/ mapping: 'fields.currency'},
    ...
    var tabcustomer_invoices = Ext.create('Ext.tab.Panel', {
        title: 'Customers Invoices',
        renderTo: Ext.getBody(),
        // activeTab: 0,
        border: false,
        items: [{
            ...
            {text: 'Customer Name', dataIndex: 'customer_name', flex: 30 / 100},
            {text: 'Invoice Description', dataIndex: 'desc', flex: 40 / 100},
            {text: 'Status', dataIndex: 'status', flex: 18 / 100, align: 'left'},
            {text: 'Amount To Pay', dataIndex: 'amountpay', xtype: 'numbercolumn', format:'0,000', flex: 18 / 100, align: 'right'},
            {text: 'Amount Paid', dataIndex: 'amountpaid', xtype: 'numbercolumn', format:'0,000', flex: 18 / 100, align: 'right'},
            {text: 'Remaining Amt', dataIndex: 'amountremain', xtype: 'numbercolumn', format:'0,000', flex: 20 / 100, align: 'right'},
            {text: 'Invoice Date', dataIndex: 'invdate', flex: 20 / 100, align: 'right'},
            {text: 'Invoice Due Date', dataIndex: 'invdue', flex: 20 / 100, align: 'right'},
            ],
            ...
            dockedItems: [{
                ...
                items: [{
                    ...
                    handler: function() {
                        ...
                        addcustomers_invoice()
                        /*tabcustomer_invoices.add(addcustomers_invoice())
                        tabcustomer_invoices.setActiveTab(addcustomers_invoice())*/
                    },
                },{
                    xtype: 'tbseparator'
                },{
                    ...
                    id: 'customerinvoicepdf',
                    disabled: true,
                    iconCls: 'pdf-icon',
                    handler: function() {
                       
                        // extraParams = { clark: 6},
                        // // var taga = tabcustomer_invoices.getComponent(0).store.getProxy({params:{clark: 2}})
                        
                        // // var newwindow = window.open(url, '_blank')
                        // window.open('/customers/report', '_blank') // print() method does a print preview    
                        
                         // window.open(url,'_blank')
                        
                        var currency = Ext.create('Ext.window.Window', {
                            title: 'Choose Currency',
                            modal: true,
                            width: 210,
                            margins:'1 -1 -1 -1',
                            layout: 'hbox',
                            animateTarget: customerinvoicepdf,
                            items: [{
                                xtype: 'combobox',
                                fieldLabel: 'Currency',
                                labelAlign: 'top',
                                ...
                                listeners: {
                                    'select': function() {
                                        var value = currency.getComponent(0)
                                        choosen_currency = value.getValue()
                                        if(choosen_currency=='US DOLLARS'){
                                            currency.setWidth(362)
                                            // currency.setHeight(300)
                                            currency.add({
                                                xtype: 'splitter',
                                                id: 'exchangesplitter',
                                                width: 15,
                                            ...
                                            currency.doLayout()
                                        }else if(choosen_currency=='Rwandan Francs'){
                                            currency.remove(Ext.getCmp('exchangerate'))
                                            currency.remove(Ext.getCmp('exchangesplitter'))
                                            currency.setWidth(210)
                            ...
                            buttons: [{
                                ...
                                handler: function(response) {
                                    if((currency.getComponent(0).getValue()) == 'Rwandan Francs'){
                                        var url = '/customers/report?invoicepkCustomer='+customer_invoice_pk+'&currency='+currency.getComponent(0).getValue();
                                        window.open(url,'_blank')
                                        currency.close()
                                    }else if(choosen_currency=='Rwandan Francs'){
                                        var url = '/customers/report?invoicepkCustomer='+customer_invoice_pk+'&currency='+choosen_currency;
                                        window.open(url,'_blank')
                                        currency.close()
                                    }else if(Ext.getCmp('exchangerate').getValue()!= null && choosen_currency=='US DOLLARS'){
                                        var url = '/customers/report?invoicepkCustomer='+customer_invoice_pk+'&currency='+choosen_currency+'&exchangerate='+Ext.getCmp('exchangerate').getValue();
                                        window.open(url,'_blank')
                                        currency.close()
                                    }else{
                                        Ext.Msg.alert('Failed', 'Exchange Rate Field Required')
                                    }
                    ...
                    listeners: {
                        'keyup': function(f,e) {
                            tabcustomer_invoices.getComponent(0).store.getProxy().extraParams={pattern: f.getValue()};
                            tabcustomer_invoices.getComponent(0).store.loadPage(1);
                        }
                    }
                }],
            },{
                ...
                items: [{
                    ...
                    xtype: 'gridpanel',
                    id: 'grid_customers_items',
                    region: 'south',
                    collapsible:true,
                    collapsed:true,
                    split: true,
                    store : customer_items,
                    title: 'Associated Items',
                    columns: [
                        
                        {text: 'Item Description', dataIndex: 'item_descri', flex: 30 / 100},
                        {text: 'Quantity', dataIndex: 'quantity', xtype: 'numbercolumn', format:'0,000', align: 'right'},
                        {text: 'Unit Price', dataIndex: 'unit_price', xtype: 'numbercolumn', format:'0,000', align: 'right'},
                        {text: 'Total exclu VAT', dataIndex: 'total_exclu_vat', xtype: 'numbercolumn', format:'0,000', align: 'right'},
                        {text: 'VAT', dataIndex: 'vat_customer', xtype: 'numbercolumn', format:'0,000', align: 'right'},
                        {text: 'Total', dataIndex: 'total_customer_item', xtype: 'numbercolumn', format:'0,000', align: 'right'},
                    ...
                    dockedItems: [{
                        ...
                        items: [{
                            ...
                            id: 'additem',
                            handler: function() {
                                addCustomerItem(customer_invoice_pk)
                            }
                        },{
                            xtype: 'button',
                            text: 'Edit item',
                            ...
                            handler: function() {
                                ...
                                editCustomerItem(item_pk, item_name, item_qty, unit_pric, connection)
                            }
                        },{
                            xtype: 'button',
                            text: 'Refresh',
                            ...
                            iconCls: 'remove-icon',
                            handler: function() {
                                removeCustomerItem(item_name, item_pk, connection)
                            }
                        }]
                    },{
                        xtype: 'toolbar',
                    ...
                    listeners: {
                        itemclick: function(data, record, position){
                            Ext.getCmp('edititem').setDisabled(false)
                            Ext.getCmp('removeitem').setDisabled(false)

                            item_pk = record.data.id
                            item_name = record.data.item_descri
                            item_qty = record.data.quantity
                            connection = record.data.invo_connect_id
                            unit_pric = record.data.unit_price
                            console.log(item_pk)
                            console.log(item_name)
                            console.log(item_qty)
                            console.log(connection)
                            console.log(unit_pric)
                        },
                        itemdblclick: function(data, record, position){
                            editCustomerItem(item_pk, item_name, item_qty, unit_pric, connection)
                        }
        ...
        listeners: {
           itemclick: function(data, record, position) {
            Ext.getCmp('editcustomers_invoice').setDisabled(false)
            Ext.getCmp('removecustomers_invoice').setDisabled(false)
            Ext.getCmp('customerinvoicepdf').setDisabled(false)
              ...
              customer_invoice_received_on = record.data.received_on
              customert_invoice_notess = record.data.cust_notes
              customer_currency = record.data.currency_inv

              

              var test = tabcustomer_invoices.getComponent(0).getDockedItems(0)[2]
              var second = test.getComponent(1)
              var items_title = customer_invoice_name+' - '+customer_currency 
              second.setTitle(items_title)
              second.store.getProxy().extraParams={invoicepkCustomer: customer_invoice_pk};
              second.store.loadPage(1);
              Ext.getCmp('grid_customers_items').expand()
           ...
           itemdblclick: function(data, record, position) {
            ...
            /*var test = tabcustomer_invoices.getComponent(0).getDockedItems(0)[2]
            var second = test.getComponent(1)
            second.setTitle(customer_invoice_name)
            second.store.getProxy().extraParams={invoicepkCustomer: cust_id};
            second.store.loadPage(1);*/
            ...
            //     method: 'POST',
            //     params: {
            //         invoicepkCustomer: cust_id,  
            //     },

            //     success: function() {
            //         Ext.Msg.alert('Items', 'Items Found');
            //         second.doLayout();
            //         console.log(second.store)
            //     },
            //     failure: function() {
                    
            //         Ext.Msg.alert('Items', 'No Items Found');
            ...
            // listeners on object
            /*if(steve.hasListeners){
                Ext.util.Observable.capture(steve, function(evname) {
                    // if(evname==itemclick){
                    //     console.log('gotit')
                    // }
                    console.log(evname);
                })
                // Ext.util.Observable.releaseCapture(steve)
            }*/

            // customers_items_inv()
                function simple(){
                    return customert_invoice_descri
                }
...
function addcustomers_invoice() {
                ...
                items: [{
                    ...
                    items: [{
                        ...
                        items: [{
                                ...
                                // queryMode: 'local',
                                width: 300,
                                name: 'customer_currency_invoice',
                                frame: true,
            ...
            dockedItems: [{
                ...
                items: ['->',{
                    ...
                    handler: function() {
                        ...
                        var customerInvoice_value = addcustomers_invoice_win.getComponent(0).getComponent(0).getValues()
                        console.log(customerInvoice_value.customer_currency_invoice)
                        Ext.Ajax.request({
                            ...
                            params: {
                                ...
                                customerCurrency: customerInvoice_value.customer_currency_invoice,
                            ...
                            success: function() {
                                ...
                                customersinvoice_store.reload() // customersstore has a global scope
   ...
   // return addcustomers_invoice_win
                ...
                items: [{
                    ...
                    items: [{
                        ...
                        items: [{
                                ...
                                // queryMode: 'local',
                                width: 300,
                                name: 'edcustomer_currency_invoice',
                                frame: true,
                                // value: invoice_status,
                    ...
                    handler: function() {
                        ...
                        Ext.Ajax.request({
                            ...
                            params: {
                                ...
                                edcustomerCurrency: edcustomerInvoice_value.edcustomer_currency_invoice,
    ...
    var suppliersgrid = Ext.create('Ext.grid.Panel', {
        ...
        dockedItems: [{
            ...
            items: [{
                ...
                listeners: {
                    'keyup': function(f,e) {
                        suppliersgrid.store.getProxy().extraParams={pattern: f.getValue()};
                        suppliersgrid.store.loadPage(1);
                    }
                }
            }],
        },{
    ...
    var suppliersinvoice_grid = Ext.create('Ext.grid.Panel', {
        ...
        dockedItems: [{
            ...
            items: [{
                ...
                listeners: {
                    'keyup': function(f,e) {
                        suppliersinvoice_grid.store.getProxy().extraParams={pattern: f.getValue()};
                        suppliersinvoice_grid.store.loadPage(1);
                    }
                }
            }],
        },{
	...
	var gridUsers = Ext.create('Ext.grid.Panel', {
        ...
        listeners: {
           itemclick: function(data, record, position) {
            ...
            Ext.getCmp('usersreport').setDisabled(false)
    ...
    var tree = Ext.create('Ext.tree.Panel', {
        ...
        listeners: {
            select: function (s, record, m) {  
               ...
               }else if(id_users === 'node2'){
                // myTab.removeAll()
                myTab.add(customers())

               }else if(id_users === 'node3'){
                // myTab.removeAll()
                myTab.add(users())
                
               }else if(id_users === 'node4'){
                // myTab.removeAll()
                myTab.add(suppliers())

               }else if(id_users === 'uprofile'){
                // myTab.removeAll()
               	myTab.add(userprofile())

               }else if(id_users === 'projects'){
                // myTab.removeAll()
               	myTab.add(projects())

               }else if(id_users === 'work'){
                // myTab.removeAll()
               	myTab.add(work())

               }else if(id_users === 'other'){
                // myTab.removeAll()
               	myTab.add(other())

               }else if(id_users === 'customers'){
                // myTab.removeAll()
               	myTab.add(customers())

               }else if(id_users === 'custInvoices'){
                // myTab.removeAll()
               	myTab.add(custInvoices())

               }else if(id_users === 'suppliers'){
                // myTab.removeAll()
               	myTab.add(suppliers())

               }else if(id_users === 'suppInvoices'){
                // myTab.removeAll()
                myTab.add(suppInvoices())

               }else if(id_users === 'usersinfo'){
                // myTab.removeAll()
               	myTab.add(users())
